...
class ApiClient
{
    ...
    public function setApiToken($token){
        $this->config->setApiKey("token", $token);
        return $this;
    }

    public function setApiSecret($secret){
        $this->config->setApiKey("secret", $secret);
        return $this;
    }
    ...
    public function callApi($resourcePath, $method, $queryParams, $postData, $headerParams, $responseType = null)
    {
        ...
        $headers = $this->processHeaders($url, $headerParams);
    ...
    /**
     * Prepare headers and sign request if all of auth headers are not present
     * @param $url
     * @param array $headers
     * @return array
     */
    public function processHeaders($url, array $headers){
        $_headers = array();
        foreach ($headers as $key => $val) {
            $_headers[] = "$key: $val";
        }

        if((!isset($headers['X-Boomtown-Date']) || !array_key_exists('X-Boomtown-Date', $headers)) ||
            (!isset($headers['X-Boomtown-Token']) || !array_key_exists('X-Boomtown-Token', $headers)) ||
            ((!isset($headers['X-Boomtown-Signature']) || !array_key_exists('X-Boomtown-Signature', $headers)))) {
            $_headers = array_merge($this->signRequest($url), $_headers);
        }
    ...
    /**
     * Generate HMAC_SHA25 signature and return the auth headers
     * @param $url
     * @return array
     */
    public function signRequest($url){
        $authHeader = [];
        ...
        if (!empty($secret) && !empty($token)) {
            ...
            $toSign = "{$path}{$query}:{$date}";

            $signature  = base64_encode(hash_hmac("sha256", $toSign, $secret, TRUE));
            $authHeader[] = "X-Boomtown-Date: $date";
            $authHeader[] = "X-Boomtown-Token: $token";
            $authHeader[] = "X-Boomtown-Signature: $signature";
        }
